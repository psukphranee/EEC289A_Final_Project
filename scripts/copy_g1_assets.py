...
def main() -> None:
    args = parse_args()
    assets_src = args.unitree_dir.resolve() / "unitree_robots" / "g1" / "assets"
    assets_dst = args.course_dir.resolve() / "g1_pg_env" / "xmls" / "assets"

    if not assets_src.is_dir():
        raise FileNotFoundError(f"G1 asset directory not found: {assets_src}")
